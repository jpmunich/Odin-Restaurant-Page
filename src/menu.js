...
function createMenu() {
    ...
    createTextElement('p', 'Pureed chick peas, tahini and lemon, garnished with parsley, olive oil and diced tomatoes', itemOne);
    const itemTwo = createMenuItem(appetizerContainer);
    createTextElement('h4', 'Garlic Feta Fries', itemTwo);
    createTextElement('p', 'French fries mixed in our homemade garlic sauce and feta cheese', itemTwo);
    const itemThree = createMenuItem(appetizerContainer);
    createTextElement('h4', 'Crispy Calamari', itemThree);
    createTextElement('p', 'Calamari seasoned just to perfection, served with spicy tahini sauce', itemThree);
    const itemFour = createMenuItem(appetizerContainer);
    createTextElement('h4', 'Spanakopita', itemFour);
    createTextElement('p', 'Classic spanakopita (spinach and cheese wrapped in phyllo dough)', itemFour);

    const itemFive = createMenuItem(entreeContainer);
    createTextElement('h4', 'Stuffed Peppers', itemFive);
    createTextElement('p', 'Red peppers stuffed with rice, garbanzo beans, tomatoes and onions in tomato sauce', itemFive);
    const itemSix = createMenuItem(entreeContainer);
    createTextElement('h4', 'Mixed Grill', itemSix);
    createTextElement('p', 'Mixed grill comes with two skewers: one lamb & one chicken served with rice and a side salad', itemSix);
    const itemSeven = createMenuItem(entreeContainer);
    createTextElement('h4', 'Gyros Platter', itemSeven);
    createTextElement('p', 'juicy gyros cooked to perfection, served with rice, side salad & tzatziki sauce', itemSeven);
    const itemEight = createMenuItem(entreeContainer);
    createTextElement('h4', 'Chicken Kabab', itemEight);
    createTextElement('p', 'Chicken tenders marinated in garlic, olive oil and Mediterranean spices', itemEight);
    const itemNine = createMenuItem(dessertContainer);
    createTextElement('h4', 'Baklava', itemNine);
    createTextElement('p', 'Homemade simple syrup, paper thin phyllo dough, butter and walnuts topped with pistachio nuts', itemNine);
    const itemTen = createMenuItem(dessertContainer);
    createTextElement('h4', 'Tiramisu', itemTen);
    createTextElement('p', 'Ladyfingers dipped in coffee, layered with a whipped mixture of egg yolks & mascarpone', itemTen);
